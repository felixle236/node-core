...
export type MockQuerySession = typeorm.QueryRunner;
 ...
 * Mock query runner
 */
export function mockQuerySession(sandbox: SinonSandbox): {connection: MockConnection, querySession: MockQuerySession} {
    const querySession = {
        startTransaction: mockFunction(),
        rollbackTransaction: mockFunction(),
        release: mockFunction()
    } as DbQuerySession;
    const connection = mockConnection(sandbox);
    connection.createQueryRunner.returns(querySession);
    sandbox.stub(querySession, 'startTransaction').resolves();
    sandbox.stub(querySession, 'rollbackTransaction').resolves();
    sandbox.stub(querySession, 'release').resolves();
    return {
        connection,
        querySession
    };
 ...
 */
export function mockWhereExpressionBuilder(): typeorm.WhereExpressionBuilder {
    const whereExpression = {} as typeorm.WhereExpressionBuilder;
